...
<p>
The trick to prevent <i>really</i> high frequencies from entering the simulation, to avoid needing a super high resolution, is to <i>slowly</i> turn on the source, rather than abruptly. In particular, there is one type of signal that is really good at only containing frequencies in a narrow window - that is the Gaussian. The Fourier Transform of a Gaussian is <a href="https://mathworld.wolfram.com/FourierTransformGaussian.html">itself a Gaussian</a> (just with an appropriate width), and it dies off like \(e^{-\omega^2}\) in frequency (really really fast), so we can be sure that we will only have a range of frequencies represented, and not a bunch of really high ones that persist a long time in our simulation and cause difficulty with resolution requirements.
